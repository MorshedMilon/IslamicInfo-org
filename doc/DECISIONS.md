...
## ADR-067 · Per-page gating is in force: 117 dua pages index under owner batch sign-off, R6 clusters adjudicated, FAQ overlap window carved out · Accepted · 2026-08-03 · Dua detail pages (indexing + validator scope)
...
Three findings carried the ruling. **The occasion bucket is our own facet, not the search intent** — C, D and F share a bucket but target distinct queries, and "what to say in ruku" is not answered by the sujood page. **Hisn itself cross-lists the wording**, and both members of each pair carry the same underlying citation (C: Riyad as-Salihin 747; D: Sunan an-Nasai 1051; F: Sunan an-Nasai 5462), so the two pages are one narration listed twice, not two competing claims. **Each pair has a real differentiator in its authored prose** — measured Jaccard overlap 0.22–0.30, with distinct openings and timing sentences naming the specific posture, moment or narrator.

**Correction, 2026-08-03 — what this ruling does NOT claim.** As first written, this ADR stated that "the narrations genuinely attest both placements", naming 'Aishah for C and 'Asim bin Humaid for D. **That went beyond the data and is withdrawn.** The corpus records the supplication text, a `hadithCitation` and Hisn's chapter assignment; it does **not** record the narration's own wording, so nothing here establishes what any narration says about a posture. What is corpus-attested is narrower and sufficient: Hisn lists the same words under two chapters, and both records carry the same citation. The authored prose was already correctly hedged — it says "the chapter assigns this to the bowing", attributing placement to the compiler and not to the narration — and this ADR now matches it. F is unaffected on this point because its limb was a claim about the **translation**, which is verifiable in the record: the wording does name "the burden of debts and from being over powered by men". The disposition is unchanged; only its stated basis is corrected. F needed the most scrutiny and resolved on its own text: `41:137`'s prose states the proportion honestly rather than overclaiming — *"Only one of the eight things named concerns debt, and it sits seventh in the list"* — so the page serves the debt query without pretending to be a debt-specific supplication.
...
**Decision 5 — the cross-listing rule, and the end of the R6 holds (owner, 2026-08-03).** Ruled: **where Hisn lists the same wording under two chapters, that cross-listing is by itself sufficient basis to index both members**, cross-linked and self-canonical. This is the generalisation of what C and D already stand on after the Correction above, and it settles clusters B (`17:34`/`19:42`) and E (`32:117`/`19:47`). Both held members are released. **Six of eight registered clusters are now adjudicated and indexed; zero pages remain held on R6; 117 of 117 authored pages index.**

The rule was applied over a recorded objection rather than in ignorance of it: B and E carry **asymmetric narrator attribution** — `17:34` and `32:117` name 'Aishah in their authored prose, while their already-live batch-3 partners (`19:42`, `19:47`) attribute placement to the chapter alone. Under this rule the chapter assignment is the operative basis, so the asymmetry does not block indexing. It is recorded on both allowlist entries so the ruling's cost stays visible.

**Two clusters are NOT resolved and are routed to the reviewer package** (`doc/DUA-REVIEWER-PACKAGE.md` Part 6), because both turn on sourcing questions this corpus cannot answer. **6.1 — `27:85`/`28:109`**: already indexed on occasion/keyword grounds, but **neither record carries any `hadithCitation`**, so both pages render a supplication with a source label and no hadith reference. The R6 question is settled; the citation gap is not, and the charter's "every claim carries a source or it is not shown" bears on it. **6.2 — `8:12`/`5:9`/`99:210`**: three pages whose entire Arabic is *Bismillah*, still **not permitted**. The cross-listing rule does not reach it — this is an incidental collision on a very short text, not a deliberate cross-listing — and it carries thin-content and cannibalisation problems independent of R6.

**Superseded note — the two pages formerly held.** The paragraph below described `17:34` and `32:117` as held pending a ruling. Decision 5 is that ruling; the paragraph is retained for the reasoning that produced the flag, and the checks it records are why the asymmetry above is documented rather than discovered later.

**Formerly held: 2 pages — `17:34` (cluster B) and `32:117` (cluster E).** Both were outside the scope given, and both are structurally the same question as C and D: B is another Ruku'/Sujood cross-listing, and E pairs Qunut in the Witr prayer against Sujood. Each has a member already live from batch 3 (`19:42`, `19:47`), so each is one ruling away from releasing one more page. **They are not resolved and must not be assumed to follow C and D — that inference is the adjudication, and it is the owner's.** Four clusters remain in `pendingAdjudication` overall, including the three-member `Bismillah` cluster (`8:12`/`5:9`/`99:210`), which carries thin-content and cannibalisation questions independent of R6.
