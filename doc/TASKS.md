...
- [ ] **BLOCKS WAVE 2 AUTHORING — implement R1–R6 as real assertions in `scripts/validate-seo.mjs`** (owner directive 2026-08-03). Copy R8's pattern: resolve paths with `P()` so the script runs from any cwd, add to `failed`, print a scanned/checked/violations block. **Each rule needs a negative control before it is trusted clean** — inject a known violation, confirm the check catches it, revert. A rule reporting 0 proves nothing until it has been seen reporting non-0; R8's own negative control found 18 stale chapter pages carrying 306 links to held pages on its first run. Then **report per rule whether it was already satisfied by luck or by design**. Working hypotheses to verify, not assume: **R3/R4 = luck** (no authored record is a `quran:` record, a Class B narration, or carries `build_gate`, so both bind nothing in the current 117 — Wave 2 is what changes that); **R6 = luck and hand-applied** (8 clusters found by a one-off audit, 6 adjudicated by hand, nothing re-checks on build); R1/R2/R5 unknown, and R2 in particular must re-check every build because flipping `gets_static_chapter_page` to `yes` is a one-character edit that silently creates a collision. Rules: `docs/seo/VALIDATOR-RULES-SESSION-1.md`. Context: ADR-067.
