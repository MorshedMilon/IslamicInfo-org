...
**Blueprint:** `hadith_module_enhanced__1_.html` · **Design System:** `CLAUDE_v3.md v3.0` · **PRD:** v1.2 · **Status:** Implementation-Ready — ONE OPEN ITEM, see §1.1 note

---

## 1. Purpose

The Hadith Library is the platform's primary scholarly reference tool. It delivers trustworthy access to 18 major hadith collections — total hadith count computed live from source data, not hardcoded (see §1.1) — with authentic grading, isnad transparency, narrator reliability panels, and a full per-hadith action suite mirroring the Qur'an Explorer.
...
### 1.1 v1.2 Open Items (not yet resolved — read before building)
...
| 3a | Sidebar | `aside.sidebar` | Collections (18 rows) · Classical Scholars · Reading Paths · Browse · Verify CTA · Ask CTA |
| 3b | Main Content | `main.main` | Stats strip → HotD → Collections grid → Topics strip → Hadith feed → Stage 2+ sections |
| 4 | Stats Strip | `.stats-strip` | [Total — compute live] Hadiths · 18 Collections · 12+ Languages · 100% Source-Verified |
| 5 | Hadith of the Day | `.daily-strip` | Arabic matn + translation + reference + grade · Bookmark · Share · View Full Isnad |
| 6 | Collections Grid | `.collections-grid` | 18 collection cards; filter tabs: All · Kutub al-Sittah · Musnad · Selected |
...
- Content: Bismillah (first child) → eyebrow badge "✦ 18 Collections · [Total — compute live] Hadiths · Fully Source-Verified" → H1 "Explore Authentic Hadith / with `<span class="gradient-italic">Scholarly Precision</span>`" → subtitle → search pill
...
- On page load: fetch all 18 collections (routed per collection to HadithAPI.com, fawazahmed0, or AhmedBaset per §5) → render `.collections-grid` cards with live hadith counts
...
- Copy button: produce exact payload:
  ...
  ```
- **Ungraded collections (the 8 AhmedBaset-sourced, per ADR-022 / §7.1):** `grade`/`grader`/`year` are `null`. Replace the entire `Grade: {grade} ({grader}, {year}).` segment with the literal string **`Grade: Not individually graded — see collection note`** (grader and year omitted). Never interpolate `null` or fabricate a grade.
...
- Server-side cache: Cloudflare KV key `hadith_explain:{hadithRef}:{lang}`, TTL 24h (no client-side AI cache on hadith page — server caches; not Redis, this project runs on Cloudflare)
...
- Alternatively: backend cron sets a daily key in Cloudflare KV `hotd:{YYYY-MM-DD}` pointing to a `hadithRef` (this project runs on Cloudflare, not Redis)
...
- `GET /api/hadith/search?q=&lang=&collection=&grade=&limit=20` → proxies to HadithAPI.com (via Cloudflare Worker, for the 9 HadithAPI-sourced collections) or self-hosted Meilisearch index (indexing all 18 collections, including the fawazahmed0/AhmedBaset-sourced ones)
...
**v1.2 correction:** the table below replaces v1.1's single-source Sunnah.com API assumption. Sunnah.com's own API was investigated and ruled out — it requires a manually-requested key and is not self-serve. Collections now route across 3 confirmed free providers: HadithAPI.com (9 collections, key required, proxied through a Cloudflare Worker so the key never reaches the client), fawazahmed0/hadith-api (1 collection, keyless, direct CDN fetch), and AhmedBaset/hadith-json (8 collections, keyless, direct fetch pinned to a release tag — license not yet confirmed, do not ship to production until resolved).

| API | Endpoint | Method | Cache TTL | Notes |
|---|---|---|---|---|
| Collections list | `/api/hadith/collections` (Cloudflare Worker) | GET | 7d | Merges live data from all 3 providers; Fallback: `/data/collections.json` |
| Books by collection (9) | `/api/hadith/{collection}/books` (Cloudflare Worker → HadithAPI.com) | GET | 7d | Tier 2 navigation, HadithAPI-sourced collections only |
| Hadiths by book (9) | `/api/hadith/{collection}/books/{book}/hadiths` (Cloudflare Worker → HadithAPI.com) | GET | 24h / (slug+book) | Tier 3a list, HadithAPI-sourced collections only |
| Single hadith (9) | `/api/hadith/{collection}/{id}` (Cloudflare Worker → HadithAPI.com) | GET | 24h | Tier 3b deep-view, HadithAPI-sourced collections only |
| 40 Hadith Nawawi | `cdn.jsdelivr.net/gh/fawazahmed0/hadith-api@1/editions/{eng\|ara}-nawawi.json` | GET (direct, no proxy) | Bundled/static | Keyless, public domain (Unlicense) |
| Riyad as-Saliheen, Bulugh al-Maram, Muwatta Malik, Adab al-Mufrad, Shamail, Darimi, both Forty collections (8) | `raw.githubusercontent.com/AhmedBaset/hadith-json/{pinned-tag}/db/...` | GET (direct, no proxy) | Bundled/static | Keyless, pinned to a release tag not `main`; license not yet confirmed |
| Related hadiths | `/api/hadith/{collection}/{id}/similar` (Cloudflare Worker) | GET | 7d | Stage 3 related section; HadithAPI-sourced collections only |
| Hadith search | `/api/hadith/search` | GET | 1h | Server proxy → HadithAPI.com (9 collections) + self-hosted Meilisearch index (all 18 collections) |
| Narrator data | `/data/narrator/{id}.json` | GET | 7d | Self-hosted; lazy on narrator panel open |
| Hadith of the Day | `/api/daily/hadith` or `/data/hotd.json` | GET | 24h | day-of-year keyed; Cloudflare KV, not Redis |
...
### 6.1 Server-Side Cache (Cloudflare KV — v1.2 correction, was Redis in v1.1)
...
| Hadith data cache | Cloudflare KV 24h; client `localStorage` keyed `ii-hadiths-{slug}-{book}-{page}`, TTL 24h; stale-while-revalidate |
...
├── public/
│   └── data/
│       ├── collections.json             # Seed: 18 collection metadata
...
│       └── illustrations/hadith/       # Stage 3: 18 collection SVG motifs
...
- Collections grid: API response → 18 cards rendered; Bukhari card has `class="featured"` + gold-aura shadow; filter tabs show/hide correct cards
