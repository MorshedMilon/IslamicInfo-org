...
## ADR-066 · ADR-054's global hold becomes a per-page gate: a page may ship when it individually satisfies Gates 1–3, evidenced per record · Proposed · 2026-08-02 · Dua detail pages (content integrity)
**Status.** **Proposed — awaiting owner approval. This is an amendment with its evidence attached, not a clearance.** Nothing has been built, indexed, approved or added to a sitemap on the strength of it. ADR-054 remains in force until this is accepted or rejected.
...
## ADR-067 · Batches 1–7 signed off: 107 dua pages index, 10 held on R6; and the FAQ overlap window excludes verbatim quoted spans · Accepted (owner instruction) · 2026-08-03 · Dua detail pages (indexing + validator scope)
**Status.** **Accepted on direct owner instruction, and already applied** — `page-copy.json`, `duas/*.html` and `sitemap.xml` are built to match. This ADR records what was done; it is not a request. One limb below is **not** accepted and is flagged for a ruling.
...
**Decision 2 — batches 1–7 are approved, and 87 pages flipped to indexable.** Applied in exactly the shape batch 3 was flipped on 2026-08-02: per entry `indexable: true`, `reviewStatus: "reviewed"`, `indexReason: approved-by-owner-2026-08-03`; per batch `approved: true` with date and approver (ADR-044). **No authored prose was edited** — the diff is confined to those three fields. Indexable pages go 20 → **107**; sitemap `<loc>` 89 → **176**, zero duplicates, every URL resolving to a file on disk.
...
**Decision 3 — 10 pages are held inside approved batches, on R6.** Six clusters have two authored members rendering the same Arabic under the R6 normalisation; all six sit in `duplicate-scripture-allowlist.pendingAdjudication`, which does not permit them. Those pages carry `indexable: false` / `indexReason: r6-duplicate-scripture-pending-adjudication` and `reviewStatus` stays `generated`, because the owner approved the batch prose but has not ruled on the collisions. Held: `27:85`, `28:109`, `17:34`, `17:35`, `19:43`, `17:37`, `19:45`, `32:117`, `34:121`, `41:137`. Where neither member was live, **both** are held — picking which page owns the query *is* the adjudication, and the allowlist states that ruling is a reviewer's, "never a code change". Where one was already live (`19:42`, `19:47`, batch 3), only the newcomer is held, preserving the existing state without a choice being made. Cluster F (`34:121` / `41:137`) is the one needing more than a rubber stamp: both sit in the **same** occasion bucket (`distress`) and compete for the same queries.

**NOT ACCEPTED — the ADR log now contradicts the shipped state, and only the owner can resolve it.** ADR-054 imposed a global hold and ADR-066 (still **Proposed**) states in terms that ADR-054 "remains in force until this is accepted or rejected", that it "authorises no other page and creates no general clearance", and that its scope is **10 named records**. The shipped state is 107 indexable pages, of which **only 7 are inside ADR-066's scope** and 100 are outside it. This drift did not begin here — batch 3's 20 pages, approved 2026-08-02, intersect ADR-066's scope in **zero** records — but this change widened it from 20 to 107. The owner holds the approval authority under ADR-044 and exercised it, so the pages ship; what is missing is the *record*. Until ADR-066 is accepted (and its scope widened) or a superseding ADR is written, `doc/DECISIONS.md` asserts a hold that `sitemap.xml` does not observe, and the content-integrity regime depends on that log being truthful. **Recommended: rule on ADR-066 before the next approval pass.**

**Also unresolved, and load-bearing.** **R1–R6 are implemented by no script.** `validate-seo.mjs` asserts §5 naming and rendered-output shape only; it does not implement the duplicate-scripture rule, the `build_gate` rule, or the Class B / `quran:` clause rule. The 10 holds above were applied by hand from a one-off audit, so the next batch approval can silently re-introduce a collision. R3 and R4 currently bind nothing in the authored set (no authored record is a `quran:` record, a Class B narration, or carries `build_gate`), which is why this passed — not because the rules ran.

**References.** ADR-044 (owner is the content approver); ADR-054 (global hold); ADR-066 (Proposed — per-page gate, scope 10); `docs/seo/VALIDATOR-RULES-SESSION-1.md` R6 (superseded section) and R7 (new); `src/data/dua/duplicate-scripture-allowlist.json`; `scripts/check-dua-a1-overlap.mjs`.
